...
We have prepared a set of data files including 124 males and 2 females to be used as an illustrative example about how to get mLRR-Y data of each sample. Files and data have been anonymized and belongs to individuals from general population. These files can be downloaded from this link [LOYdata](https://goo.gl/tZu2Pt). 


The zipped file includes one file per sample in the required format to be processed with `MADloy` package. This format is described in the next section. Decompress the files in a folder and set this folder path in an R object (for instance `rawDataPath`). 
