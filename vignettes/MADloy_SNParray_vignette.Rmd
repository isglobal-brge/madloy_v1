...
We have prepared a set of data files including 124 males and 2 females to be used as an illustrative example about how to get mLRR-Y data of each sample. Files and data have been anonymized and belongs to individuals from general population. These files are encapsulated into our package `LOYdata` that can be downloaded from this link [LOYdata](https://goo.gl/tZu2Pt) (NOTE: a GitHub repository is being created):

Then, the package can be installed into your computer by executing (NOTE: before executing thi coed, set working directory into the one having the downloaded packages):

```{r eval=FALSE}
install.packages("LOYdata_0.1.0.tar.gz", repos=NULL, 
                 type="source")
```

The package includes one file per sample in the required format of `madloy` that is described in the next section. The path indicating where the raw data (pennCNV format) is available can be obtained by executing:


```{r, get_data_path}
# rawDataPath <- system.file("extdata", "rawData", package = "LOYdata")
rawDataPath <- "C:/Juan/CREAL/MADloy/rawData"
rawDataPath
...
LOY association studies are perfomed only using male samples. Obviously, clinical data can be used to filter such individuals. However, in some ocassions there are errors in those databases that can be detected by using genomic data. The function `checkSex` can be used to further verify that there are no female samples in our data. Let us perform this filtering using HapMap data 

```{r, check_sex, cache=TRUE}
sex <- checkSex(rawDataPath)
```
This function only requires the path containing the raw data in pennCNV format. By default the function is assuming that the LRR information is in column number 4. This can be changed through the argument `LRRCol`. 
...
> * The sample mean-trimed LRR values in autosomes
> * The median value of summarized mLRR-Y region of all samples 


The `madloy` function is design to process LRR files and only requires the path where those files are located. Let us illustrate how to get summarized data of our illustrative example available at `LOYdata` package (see Getting Started section). 

```{r, madloy_example, cache=TRUE}
ex <- madloy(rawDataPath)
```
...
```{r, fig.show='hold',  fig.cap = "Plot of MADloy object of 124 samples"}
plot(ex, print.labels=TRUE, threshold=-0.3)
```


This figure shows several samples that may have a LOY event (those in the -2, -4 range of Y-axis). These samples can be further analyzed by looking at the mLRR-Y region to verify whether they are real LOY rearrangements as described in the next section. 


# Visualizing mLRR-Y region of a single sample

We can visually inspect the information of mLRR-Y region (e.g., one by one sample) and decide whether a given individulas is having a LOY or not. This can be performed either by using  `plotIndSNP` or `plotIndLRR` functions. Let's create these plot of a given sample having a normallized mLRR-Y value arond 0 (i.e SAMPLE_1). The figure represents the expected values of normal LRR at 0 (red line). However, as previously mentioned, depending the algorithm used to get LRR in the mLRR-Y region these values can be centered around -0.46 indicating that only 1 chromosome is present in males. Orange line represents the median value of mLRR-Y in the subset of analyzed samples. This is the value that must be consided as the reference to indicate whether the mLRR-Y is a LOY event or not.  

```{r, plotSample1,  fig.cap="LRR and BAF in the mLRR-Y region (shaded) of Sample1 from illustrative example"}
plotIndSNP(ex, sample="SAMPLE_1")
````


```{r, plotSample1_LRR,  fig.cap="LRR in the mLRR-Y region (shaded) of Sample1 from the illustrative example"}
plotIndLRR(ex, sample="SAMPLE_1")
````


Now, let us create the plot of SAMPLE_21 and SAMPLE_4 that are having a clear LOY. This is reflected by observing that LRR (black dots) in the mLRR-Y region (shaded area) is far from the reference (orange line).



```{r, plotSample2,  fig.cap="LRR in the mLRR-Y region (shaded) of SAMPLE_4 and SAMPLE_21 from the illustrative example"}
plotIndLRR(ex, sample="SAMPLE_4")
